...
DataList * FindMemory(MemoryList *head, char *scope, char *name){	// Encontrar a variável
    ...
    pointer = head;
    ap = pointer->data;
    while(ap != NULL)	//procura primeiro no global
    {		
...
void GenAssembly(QuadList *quad, MemoryList *head, AssemblyList *A)	// Funcao que gera o codigo assembly
{
    ...
    case STORE:
        ...
        {
            if (strcmp(quad->arg2, "-") == 0){
                InsereAssembly(A, "STORE", "$gp", quad->arg1, "", 0, line, 2, "");
                line++;
            }
            else 
            {
    ...
    case FUNC:	// Inicio de funcao
        ...
        if(strcmp(scope,"main") != 0){	//caso a funcao nao seja a main e necessario guardar a ra na memoria em caso de recursao
             InsereAssembly(A, "STORE", "$ra", "$sp", "", 0, line, 2, "");
             line++;
             spPos++;
        }
        break;
...
AssemblyList * AssemblyGenerator(QuadList * Q){
	AssemblyList *headA;
	MemoryList *headM;
