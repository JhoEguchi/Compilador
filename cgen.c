...
static void cGenWoS (TreeNode * tree);
  ...
  switch (tree->kind.exp) {
    ...
    case CallK:
         ...
         if(strcmp(tree->attr.name, "output") == 0)
         	...
         	newQuad(Q, CALL, getString("$t%d",IncrementaReg()), tree->attr.name, getString("%d",contador));
         Q = Q->next;
         break;
    ...
    case IdK :
         ...
         if(p1 == NULL){
         	...
         	newQuad(Q, LOAD, getString("$t%d", IncrementaReg()), tree->attr.name, t1);
         	Q = Q->next;
         }
      break; /* IdK */
    case TypeK :
    ...
    case OpK :
         ...
         t1 = getString("$t%d", IncrementaReg());
